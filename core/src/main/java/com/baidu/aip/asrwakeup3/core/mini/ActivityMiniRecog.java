...
public class ActivityMiniRecog extends AppCompatActivity implements EventListener{
   ...
   protected String final_result="暂无数据";
   // private static String DESC_TEXT = "精简版测试语音\n" ;

    protected Handler handler_vocal;
            ...
            if (params.contains("\"nlu_result\"")) {
              ...
              //  logTxt += ", 临时识别结果：" + params;
               ...
               // logTxt += ", 最终识别结果：" + params;
                ...
                try {
                    final_result = params.substring(params.indexOf("[\"") + 2, params.indexOf("]")-1);
                } catch (Exception e) {
                    e.printStackTrace();
                    final_result="精简版少说点话！";
                }
                Log.i("final_result", final_result);
                Message msg = new Message();
                msg.what = 1;
                msg.obj =final_result;
                // 没有用到obj字段
                handler_vocal.sendMessage(msg);
               // Toast.makeText(ActivityMiniRecog.this, "识别结果："+final_result, Toast.LENGTH_LONG).show();
            }  else {
                // 一般这里不会运行
